        ...
        <div className='mt-6'>
          <div className='bg-white rounded-xl shadow p-1 flex max-w-md mx-auto'>
            {["posts","media","likes"].map((tab)=>(
              <button onClick={()=>setActiveTab(tab)} key={tab} className={`flex-1 px- py-2 text-sm font-medium
                ...
                {posts.filter((post)=> post.image_urls.length >0 ).map((post)=>(
                  ...
                  {post.image_urls.map((img, index)=>(
                    <Link target='_blank' to={img} key={index} className='relative group'>
                      <img src={img} key={index} className='w-64 aspect-video object-cover' alt="" />
