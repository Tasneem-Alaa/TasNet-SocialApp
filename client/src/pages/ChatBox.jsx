...
const ChatBox = () => {
  ...
  const fetchUserMessages = async ()=>{
    try {
      ...
      dispatach(fetchMessages({token,userId}))
    } catch (error) {
  ...
  const sendMessage = async ()=>{
    try {
      ...
      if(data.success){
        setText('')
        setImage(null)
        dispatach(addMessages(data.message))
      }else{
  ...
  return user && (
    <div className='flex flex-col h-screen'>
      <div className='flex items-center gap-2 md:px-10 xl:pl-42
        ...
        <div className='space-y-4 max-w-4xl mx-auto'>
          {
            ...
            .map((msg,i)=>(
              <div key={i} className={`flex flex-col ${msg.to_user_id !==
                user._id ? 'items-start' : 'items-end'}`}>
                  <div className={`p-2 text-sm max-w-sm bg-white text-slate-700
                    rounded-lg shadow ${msg.to_user_id !==
                    user._id ? 'rounded-bl-none' : 'rounded-br-none'}`}>
                    {
                    msg.message_type==='image' &&
                    <img src={msg.media_url} className='w-full max-w-sm rounded-lg mb-1' alt="" />
                    }
                    <p>{msg.text}</p>
                  </div>
              </div>
      ...
      <div className='px-4'>
          ...
          max-w-xl mx-auto border border-gray-200 shadow rounded-full mb-5'>
            ...
            <label htmlFor="image">
              {image ? <img src={URL.createObjectURL(image)} className='h-7 rounded'/> : <ImageIcon className='size-7 text-gray-400 cursor-pointer'/>}
              <input type="file" id='image' accept='image/*' hidden 
              onChange={(e)=>setImage(e.target.files[0])}/>
            </label>
            <button onClick={sendMessage} className='p-2 rounded-full bg-linear-to-r from-purple-800 to-pink-400 text-white font-bold 
